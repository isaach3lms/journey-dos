...
from app.stages import CONTACT_WINDOW_DAYS, STAGE_BY_CODE, STAGES, TRANSITIONAL_STAGES
...
class TestWhatCountsAsStuck:
    ...
    def test_overdue_and_silent_is_stuck(self, db):
        person = add_person(db, "journey", "Stuck", "Guest", "visitor", 60, None)
    ...
    def test_the_boundary_is_strictly_past_the_expectation(self, db):
        """Read off the stage rather than typed in, so retiring or retiming a
        stage cannot leave this passing against a number nobody uses."""
        days = STAGE_BY_CODE["visitor"].expected_days
        exactly = add_person(db, "journey", "Exactly", "OnIt", "visitor", days, None)
        assert not exactly.is_stuck
        one_more = add_person(db, "journey", "One", "Over", "visitor", days + 1, None)
        assert one_more.is_stuck

    def test_the_contact_window_boundary(self, db):
        inside = add_person(
            db, "journey", "Just", "Inside", "visitor", 60, CONTACT_WINDOW_DAYS
        )
        assert not inside.is_stuck
        outside = add_person(
            db, "journey", "Just", "Outside", "visitor", 60, CONTACT_WINDOW_DAYS + 1
        )
...
class TestTheQueryMatchesTheProperty:
    ...
    def test_every_person_agrees(self, db):
        add_person(db, "journey", "A", "Stuck", "visitor", 60, None)
    ...
    def test_the_count_matches_the_list(self, db):
        for i in range(4):
            add_person(db, "journey", f"P{i}", "Stuck", "visitor", 60, None)
        church = db.session.scalar(db.select(Church).where(Church.slug == "journey"))
    ...
    def test_archived_people_never_flag(self, db):
        person = add_person(db, "journey", "Gone", "Away", "visitor", 60, None)
    ...
    def test_the_flag_is_scoped_to_one_church(self, db):
        add_person(db, "journey", "J", "Stuck", "visitor", 60, None)
        add_person(db, "riverbend", "R", "Stuck", "visitor", 60, None)
...
class TestLoggingContactClearsTheFlag:
    ...
    def test_end_to_end(self, db, staff):
        person = add_person(db, "journey", "Marcus", "Webb", "visitor", 60, None)
        assert person.is_stuck
    ...
    def test_last_contact_only_moves_forward(self, db, staff):
        ...
        person = add_person(db, "journey", "Marcus", "Webb", "visitor", 60, 2)
    ...
    def test_contact_cannot_be_logged_across_churches(self, db, staff):
        stranger = add_person(db, "riverbend", "Other", "Person", "visitor", 60, None)
        r = staff.post(
...
class TestDenormalizedColumnStaysHonest:
    def test_recompute_rebuilds_from_the_log(self, app, db):
        person = add_person(db, "journey", "Drift", "Case", "visitor", 60, None)
    ...
    def test_recompute_clears_a_value_with_no_log_behind_it(self, app, db):
        person = add_person(db, "journey", "Phantom", "Contact", "visitor", 60, 3)
...
class TestDashboard:
    def test_the_flagged_card_shows_stuck_people(self, db, staff):
        add_person(db, "journey", "Camila", "Reyes", "visitor", 60, None)
    ...
    def test_the_card_says_so_when_nobody_is_stuck(self, db, staff):
        add_person(db, "journey", "Fine", "Person", "visitor", 60, 2)
        r = staff.get("/", headers={"Host": JOURNEY_HOST})
